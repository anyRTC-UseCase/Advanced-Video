...
class ARVideoViewLayouter {
    
    fileprivate var layoutConstraints = [NSLayoutConstraint]()
    
    func layout(sessions: [ARVideoSession], fullSession: ARVideoSession?, inContainer container: UIView) {
    ...
    func responseSession(of gesture: UIGestureRecognizer, inSessions sessions: [ARVideoSession], inContainerView container: UIView) -> ARVideoSession? {
...
private extension ARVideoViewLayouter {
    func viewList(from sessions: [ARVideoSession], maxCount: Int, ignorSession: ARVideoSession?) -> [UIView] {
        var views = [UIView]()
        for session in sessions {
